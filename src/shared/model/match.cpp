...
void Match::DeserializeAndUpdate(string serialized) {
	if (serialized.length() < 282) {
		Logger::getInstance()->error("(Match:DeserializeAndUpdate) Se recibió string con longitud menor a 282");
		Logger::getInstance()->error(serialized);
		return;
	} else {
