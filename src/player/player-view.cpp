...
void PlayerView::Render(int x_camera, int y_camera, int max_x, int max_y) {
    ...
    if (player->IsSelected()) {
        ...
        //current_animation_index = PLAYER_SELECTOR_ANIMATION_INDEX;

        //SDL_Rect* current_clip = player_selector_clips;

        SpriteSheet* selector = new SpriteSheet(renderer, "seleccion3.png", player_selector_clips);
        selector->Render(x, y, player_selector_sprite_1, 0);
    }
